...
describe('Client-side Auth Service', () => {
  ...
  describe('OAuth Flow', () => {
    ...
    test('should handle OAuth callback with proper parameters', async () => {
      ...
      expect(mockAuthService.saveAuthState).toHaveBeenCalledWith(expect.objectContaining({
        token: 'test-token',
    ...
    test('should reject callback with invalid state', async () => {
      ...
      await expect(mockAuthService.handleCallback('test-code', 'wrong-state'))
  ...
  describe('Token Exchange', () => {
    ...
    test('should handle proxy errors gracefully', async () => {
      ...
      await expect(mockAuthService.exchangeCodeForToken('invalid-code'))
