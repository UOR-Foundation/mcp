...
export class PubSubManager {
   ...
   */
  updateSubscription(
    subscription: ChannelSubscriptionObject,
  ...
  async publishEvent(
    ...
    console.log(
      `Publishing event ${event.id} to channel ${channel.id} with ${subscribers.length} subscribers`
    );
      ...
      for (const [eventId, records] of this.deliveryQueue.entries()) {
        ...
        for (const record of pendingRecords) {
          try {
            ...
            console.error(
              `Error delivering event ${eventId} to subscriber ${record.subscriberId}:`,
              error
            );
  ...
  async propagateEventAcrossNamespaces(
    ...
    console.log(
      `Propagating event ${event.id} from ${sourceNamespace} to ${targetNamespaces.join(', ')}`
    );
      ...
      try {
        ...
        const targetEvent = this.createEvent(`${targetNamespace}/${event.id}`, {
          ...
          metadata: {
            ...
            resolutionPath: resolution.path.steps
              .map((step: { from: string; to: string }) => step.from)
              .join(','),
          },
        });
