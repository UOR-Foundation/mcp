...
export class IPFSStorageProvider extends BaseStorageProvider {
  ...
  private async getCIDReference(
    ...
    try {
      ...
      const reference = await this.githubProvider.getObject(username, 'ipfs_reference', `${type}_${id}`);
      
      if (!reference || !reference.reference || !reference.reference.cid) {
        return null;
      }
      
      return reference.reference.cid;
    } catch (error) {
   ...
   * @param id Object ID
   */
  private async deleteCIDReference(
    username: string, 
    type: string, 
    id: string
  ): Promise<boolean> {
    try {
      ...
      const result = await this.githubProvider.deleteObject(username, 'ipfs_reference', `${type}_${id}`);
  ...
  private async getMetadataReference(
    ...
    try {
      ...
      if (!reference || !reference.reference || !reference.reference.metadata) {
        return null;
      }
      
      return reference.reference.metadata;
    } catch (error) {
