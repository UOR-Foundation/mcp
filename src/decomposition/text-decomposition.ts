...
export class TextDecompositionAlgorithm extends BaseDecompositionAlgorithm {
  ...
  decompose(text: string): PrimeDecomposition {
    ...
    uniqueChars.forEach(char => {
      ...
      factors.push(this.createPrimeFactor(`char-${charIndex++}`, { character: char }, count));
    });
    ...
    words.forEach(word => {
      if (word.trim().length > 0) {
        factors.push(this.createPrimeFactor(`word-${wordIndex++}`, { word: word }));
      }
    });
    ...
    sentences.forEach(sentence => {
      if (sentence.trim().length > 0) {
        factors.push(this.createPrimeFactor(`sentence-${sentenceIndex++}`, { sentence: sentence }));
      }
    });
    ...
    paragraphs.forEach(paragraph => {
      if (paragraph.trim().length > 0) {
        factors.push(
          this.createPrimeFactor(`paragraph-${paragraphIndex++}`, { paragraph: paragraph })
        );
      }
    });
