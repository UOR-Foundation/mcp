...
export class StructuredDataDecompositionAlgorithm extends BaseDecompositionAlgorithm {
  ...
  decompose(data: any): PrimeDecomposition {
    ...
    if (parsedData.$schema) {
      factors.push(this.createPrimeFactor('schema', { schema: parsedData.$schema }));
    }
  ...
  private processStructure(data: any, path: string, factors: PrimeFactor[]): void {
    if (Array.isArray(data)) {
      factors.push(
        this.createPrimeFactor(`array-${path || 'root'}`, {
          path,
          type: 'array',
          length: data.length,
        })
      ...
      data.forEach((item, index) => {
        ...
        if (typeof item === 'object' && item !== null) {
          factors.push(
            this.createPrimeFactor(`property-${itemPath}`, {
              path,
              key: index,
          ...
          factors.push(
            this.createPrimeFactor(`value-${itemPath}`, {
              path,
              key: index,
              value: item,
      ...
      factors.push(
        this.createPrimeFactor(`object-${path || 'root'}`, {
          path,
          type: 'object',
      ...
      Object.entries(data).forEach(([key, value]) => {
        ...
        if (typeof value === 'object' && value !== null) {
          factors.push(
            this.createPrimeFactor(`property-${propPath}`, {
              path,
              key,
          ...
          factors.push(
            this.createPrimeFactor(`value-${propPath}`, {
              path,
              key,
              value,
  ...
  private calculateStructuralCoherence(data: any): number {
    ...
    if (Array.isArray(data)) {
      ...
      const avgElementCoherence =
        elementCoherences.reduce((sum, val) => sum + val, 0) / elementCoherences.length;
      ...
      const valueTypes = new Set(
        keys.map(key => (Array.isArray(data[key]) ? 'array' : typeof data[key]))
      );
      const typeCoherence = 1 / valueTypes.size;

      const propertyCoherences = keys.map(key => this.calculateStructuralCoherence(data[key]));
      const avgPropertyCoherence =
        propertyCoherences.reduce((sum, val) => sum + val, 0) / propertyCoherences.length;
