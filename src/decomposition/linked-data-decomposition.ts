...
export class LinkedDataDecompositionAlgorithm extends BaseDecompositionAlgorithm {
  ...
  recompose(decomposition: PrimeDecomposition): LinkedData {
    ...
    const nodeFactors = decomposition.primeFactors.filter(
      factor => factor.id.startsWith('node-')
    );
    ...
    const edgeFactors = decomposition.primeFactors.filter(
      factor => factor.id.startsWith('edge-')
    );
    ...
    const metadataFactor = decomposition.primeFactors.find(
      factor => factor.id === 'metadata'
    );
  ...
  private extractLinkedData(data: any): LinkedData {
    ...
    if (Array.isArray(data) && data.length > 0 && data[0].subject && data[0].predicate && data[0].object) {
